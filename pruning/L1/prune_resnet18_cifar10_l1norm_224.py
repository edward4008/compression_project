...
def get_dataloader_cifar10_224():
...
def train_model(model, train_loader, test_loader):
    ...
    for epoch in range(args.total_epochs):
        ...
        if best_acc<acc:
            torch.save( model, 'resnet18-round%d-224.pth'%(args.round) )
            best_acc=acc
        scheduler.step()
        torch.save( model, 'resnet18-round%d-last-224.pth'%(args.round) )
...
def main():
    train_loader, test_loader = get_dataloader_cifar10_224()
    if args.mode=='train':
        args.round=0
        model = resnet.resnet18(num_classes=10)
        train_model(model, train_loader, test_loader)
    elif args.mode=='prune':
        previous_ckpt = 'resnet18-round%d-224.pth'%(args.round-1)
        ...
        ckpt = 'resnet18-round%d-224.pth'%(args.round)
