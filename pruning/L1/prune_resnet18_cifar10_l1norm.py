...
def train_model(model, train_loader, test_loader):
    ...
    for epoch in range(args.total_epochs):
        ...
        if best_acc<acc:
            torch.save( model, 'resnet18_cifar10_32_norm/resnet18-round%d-best.pth'%(args.round) )
            best_acc=acc
            best_ece = ece
            best_epoch = epoch+1
        scheduler.step()
        torch.save( model, 'resnet18_cifar10_32_norm/resnet18-round%d-last.pth'%(args.round) )
