    ...
    for epoch in range(args.total_epochs):
        ...
        for i, (img, target) in enumerate(train_loader):
            ...
            if i%10==0 and args.verbose:
                print("Epoch %d/%d, iter %d/%d, loss=%.4f"%(epoch+1, args.total_epochs, i, len(train_loader), loss.item()))
        model.eval()
        acc, ece = eval(model, test_loader)
        print("Epoch %d/%d, Acc=%.4f"%(epoch+1, args.total_epochs, acc))
        if best_acc<acc:
            torch.save( model, 'resnet50_cifar10_32_norm/resnet50-round%d.pth'%(args.round) )
            best_acc=acc
            best_ece = ece
            best_epoch = epoch+1
        scheduler.step()
        torch.save( model, 'resnet50_cifar10_32_norm/resnet50-round%d-last.pth'%(args.round) )
...
def main():
    ...
    size = 32
    b = 256
    train_loader, test_loader = get_dataloader_cifar(dataset, size, b)
    if args.mode=='train':
        args.round=0
        ...
        ckpt = 'resnet50-round%d.pth'%(args.round)
